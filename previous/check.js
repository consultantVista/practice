function checkListsNumberValidity()
{
    let response = ``;
...
function checkListsValidity()
{
    ...
    for (let i = 0; i < lists.length; i++)
    {
        ...
        let minimumSeats = parseFloat(numberOfSeats) * parseFloat(0.4);
        if (parseInt(lists[i].numberOfCandidates) < 3 ||  parseInt(lists[i].numberOfCandidates) < minimumSeats)
        {
            let j = i + 1;
            let min;
            if (minimumSeats > 3)
            ...
            response += `The number of candidates for List #${j} should be at least ${parseInt(min)}.`;
            response += '<br/>';
        }
    }    
    return response;       
...
function checkCandidatesValidity()
{
    let response = ``;
	for (let i = 0; i < nLists; i++)
	{
		for (let j = 0; j < lists[i].numberOfCandidates; j++)
		{
            ...
            let candidateSecDistrict = lists[i].candidates[j].secDistrict.replace(/\s/g, '') 
            let ii = i + 1;
            let jj = j + 1;
            if (candidateName == '')
            {
