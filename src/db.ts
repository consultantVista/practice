  ...
  getSeatsNumberByName: function(x) {
    ...
    for (let i = 0; i < 26; i++) {
      if (this.array[i].name == x) {
        y =
          y +
          this.array[i].Sunnites +
          this.array[i].Shiites +
          this.array[i].Druze +
          this.array[i].Alawites +
          this.array[i].Maronites +
          this.array[i].Orthodox +
          this.array[i].Catholics +
          this.array[i].Protestants +
          this.array[i].Minorities +
          this.array[i].Armenian_Orthodox +
          this.array[i].Armenian_Catholics;
        return y;
      }
    }
  },
...
export let district = {
  ...
  getSeatsDistributionByName: function(x) {
    ...
    for (let i = 0; i < 26; i++) {
      if (secDistrict.array[i].district == x) {
        y.push(
          secDistrict.getSeatsDistributionByName(secDistrict.array[i].name)
        );
      }
    }
    let z = [];
